...
import { generateNextPeriod } from "../api/api";
  ...
  const totalMinutes =
    plan?.schedule?.reduce((sum, day) => {
      return (
        sum +
        (day.totalMinutes ||
          day.studyBlocks?.reduce((s, b) => s + b.duration, 0) ||
          0)
      );
    }, 0) || 0;
  ...
  const handleGenerateNext = async () => {
    ...
    try {
      ...
      if (response.success && response.plan) {
        ...
        alert(
          `New week generated! Completion rate from last week: ${response.analysis.completionRate}%`
        );
      }
    } catch (error) {
